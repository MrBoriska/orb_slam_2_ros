...
void Optimizer::GlobalBundleAdjustmentNavState(
        Map* pMap,
        const cv::Mat& gw,
        const cv::Mat& Tbc_Mat,
        int nIterations,
        bool* pbStopFlag,
...
//    Matrix4d Tbc = ConfigParam::GetEigTbc();
    Matrix4d Tbc = Converter::toMatrix4d(Tbc_Mat);
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        else
        {
            ...
            cv::Mat Twc_ = Twb_* Tbc_Mat;   // ConfigParam::GetMatTbc();
...
//    pFrame->UpdatePoseFromNS(ConfigParam::GetMatTbc());
    pFrame->UpdatePoseFromNS(Tbc_Mat);
...
void Optimizer::LocalBundleAdjustmentNavState(
        KeyFrame *pCurKF,
...
//    g2o::OptimizationAlgorithmGaussNewton* solver = new g2o::OptimizationAlgorithmGaussNewton(solver_ptr);
    ...
    // Add unary edges for gyro bias vertex
//    std::cout << "Begin Add unary edges for gyro bias vertex" << std::endl;
    KeyFrame* pPrevKF0 = vpKFs.front();

    // Test Log
    if(NULL==pPrevKF0)
        std::cout << "pPrevKF0 is NULL" << std::endl;
...
//        KeyFrame* pKF = *lit;
//        if(NULL == pKF)
//            std::cout << "pKF is NULL" << std::endl;

//        KeyFrame* pPrevKF = pKF->GetPrevKeyFrame();

//        std::cout <<  "pKF->mnId = " << pKF->mnId ;
//        if(NULL==pPrevKF)
//            std::cout << ", prev KeyFrame is NULL, why? " << std::endl;
//        else
//            std::cout << std::endl;
//    }
    ...
    for(std::vector<KeyFrame*>::const_iterator lit=vpKFs.begin(), lend=vpKFs.end(); lit!=lend; lit++)
    {
        ...
        static int k=1;
        if(NULL==pPrevKF)
            std::cout <<  "pKF->mnId = " << pKF->mnId  << ", prev KeyFrame is NULL, k = " << k++ << std::endl;
...
//    std::cout << "Finish Add unary edges for gyro bias vertex" << std::endl;
...
Vector3d Optimizer::OptimizeInitialGyroBias(const std::vector<KeyFrame *> &vpKFs, const cv::Mat& Rcb_Mat)
{
    //size_t N = vpKFs.size();
//    Matrix4d Tbc = ConfigParam::GetEigTbc();
//    Matrix3d Rcb = Tbc.topLeftCorner(3,3).transpose();

    Matrix3d Rcb = Converter::toMatrix3d(Rcb_Mat);
...
void Optimizer::OptimizeEssentialGraphForMonoVI(
        ...
        LoopClosing* pLC)
{
    ...
    cv::Mat Tbc = Tbc_Mat; // ConfigParam::GetMatTbc();     // for VI-ORB_SLAM
...
void Optimizer::LocalBundleAdjustment(
        ...
        LocalMapping* pLM)
{
//    cout << "4---Execute LocalBundleAdjustment";
    ...
    {
        pLM->SetMapUpdateFlagInTracking(true);
//        cout << " SetMapUpdateFlagInTracking(true)" << " GetMapUpdateFlagForTracking() = " << pLM->GetMapUpdateFlagForTracking() << endl;
    }
//    else
//        cout << endl;
}
