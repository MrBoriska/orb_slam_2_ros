...
void IMUPreintegrator::update(const Vector3d& omega, const Vector3d& acc, const double& dt)
{
    ...
    Matrix3d Jr = JacobianR(omega*dt);  // 测量值相对于测量噪声是大量

    // noise covariance propagation of delta measurements
    /// 对应于Eq.(63) - TRO2017:"On-Manifold Preintegration for Real-Time Visual-Inertial Odometry"
    ...
    /// 对应于Eq.(36) - TRO2017:"On-Manifold Preintegration for Real-Time Visual-Inertial Odometry"
...
//        Ca*IMUData::getAccMeasCov()*Ca.transpose();
    ...
    // delta time
    _delta_time += dt;  // 预计分计算的累计时间,相当于 delta_t_ij, delta_t_ij=sum_i_j(dt)
