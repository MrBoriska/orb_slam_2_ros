...
void MsgSynchronizer::RGBImageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    addRGBImageMsg(msg);
}

void MsgSynchronizer::DepthImageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    addDepthImageMsg(msg);
}

void MsgSynchronizer::addRGBImageMsg(const sensor_msgs::ImageConstPtr &imgmsg)
{
    _RGBImageMsgQueue.push(imgmsg);
}

void MsgSynchronizer::addDepthImageMsg(const sensor_msgs::ImageConstPtr &imgmsg)
{
    _DepthImageMsgQueue.push(imgmsg);
}


bool MsgSynchronizer::getRecentRGBMsgs(sensor_msgs::ImageConstPtr &imgmsg)
{   
    if (_RGBImageMsgQueue.empty())
        return false;

    imgmsg = _RGBImageMsgQueue.front();    // 获取第一个元素
    _RGBImageMsgQueue.pop();   // 清除第一个元素

    return true;
}

bool MsgSynchronizer::getRecentDepthMsgs(sensor_msgs::ImageConstPtr &imgmsg)
{
    if (_DepthImageMsgQueue.empty())
        return false;

    imgmsg = _DepthImageMsgQueue.front();    // 获取第一个元素
    _DepthImageMsgQueue.pop();   // 清除第一个元素
...
//        ROS_WARN("no imu message stored, shouldn't");
    ...
    // get image message
    imgmsg = _imageMsgQueue.front();    // 获取第一个元素
    _imageMsgQueue.pop();   // 清除第一个元素
...
{
//    std::cout << "call MsgSynchronizer::addImageMsg " << std::endl;
//    std::cout << "image timestamp is: " << imgmsg->header.stamp.toSec()   << std::endl;
    if(_imageMsgDelaySec >= 0) {
